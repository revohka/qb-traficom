...
Citizen.CreateThread(function()
    while true do
        ...
        if hori ~= nil then
            if IsPedInAnyVehicle(GetPlayerPed(-1), false) and GetPedInVehicleSeat(GetVehiclePedIsIn(GetPlayerPed(-1), true), -1) == GetPlayerPed(-1) then
                ...
                if LastVehicle ~= GetVehiclePedIsIn(GetPlayerPed(-1), false) then
                    hori.Functions.TriggerCallback('vehiclekeys:CheckHasKey', function(result)
                        if result then
                            HasKey = true
        ...
        if not HasKey and IsPedInAnyVehicle(GetPlayerPed(-1), false) and GetPedInVehicleSeat(GetVehiclePedIsIn(GetPlayerPed(-1), false), -1) == GetPlayerPed(-1) and hori ~= nil and not IsHotwiring then
            ...
            hori.Functions.DrawText3D(vehpos.x, vehpos.y, vehpos.z, "~g~H~w~ - Hotwire")
            SetVehicleEngineOn(veh, false, false, true)
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(7)
        if not IsRobbing and isLoggedIn and hori ~= nil then
            if GetVehiclePedIsTryingToEnter(GetPlayerPed(-1)) ~= nil and GetVehiclePedIsTryingToEnter(GetPlayerPed(-1)) ~= 0 then
                ...
                if driver ~= 0 and not IsPedAPlayer(driver) then
                    if IsEntityDead(driver) then
                        IsRobbing = true
                        hori.Functions.Progressbar("rob_keys", "Grabing keysn..", 3000, false, true, {}, {}, {}, {}, function()
            ...
            if hori.Functions.GetPlayerData().job.name ~= "police" then
...
RegisterNetEvent('hori:Client:OnPlayerLoaded')
AddEventHandler('hori:Client:OnPlayerLoaded', function()
    isLoggedIn = true
end)

RegisterNetEvent('hori:Client:OnPlayerUnload')
AddEventHandler('hori:Client:OnPlayerUnload', function()
    isLoggedIn = false
end)
...
AddEventHandler('vehiclekeys:client:SetOwner', function(plate)
    ...
    HasKey = true
    hori.Functions.Notify('You picked the keys of the vehicle', 'success', 3500)
end)
...
function LockVehicle()
    local veh = hori.Functions.GetClosestVehicle()
    ...
    if veh ~= nil and GetDistanceBetweenCoords(pos.x, pos.y, pos.z, vehpos.x, vehpos.y, vehpos.z, true) < 7.5 then
        hori.Functions.TriggerCallback('vehiclekeys:CheckHasKey', function(result)
            if result then
                ...
                hori.Functions.Notify('Sinulla ei ole ajoneuvon avaimia..', 'error')
            end
        end, plate)
    end
end
...
function LockpickDoor(isAdvanced)
    local vehicle = hori.Functions.GetClosestVehicle()
    if vehicle ~= nil and vehicle ~= 0 then
        ...
        if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, vehpos.x, vehpos.y, vehpos.z, true) < 1.5 then
            ...
            if (vehLockStatus > 1) then
                ...
                hori.Functions.Progressbar("lockpick_vehicledoor", "Murto käynnissä..", lockpickTime, false, true, {
                    ...
                    if math.random(1, 100) <= 90 then
                        hori.Functions.Notify("Onnistuit..")
                        TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 5, "unlock", 0.3)
                        SetVehicleDoorsLocked(vehicle, 0)
                        SetVehicleDoorsLockedForAllPlayers(vehicle, false)
                    else
                        hori.Functions.Notify("No voi vittu..", "error")
                    end
                end, function()
                    openingDoor = false
                    StopAnimTask(GetPlayerPed(-1), "veh@break_in@0h@p_m_one@", "low_force_entry_ds", 1.0)
            ...
            if GetPedInVehicleSeat(vehicle, -1) == GetPlayerPed(-1) then
                ...
                Skillbar.Start({
                    ...
                    if IsHotwiring then
                        ...
                        HasKey = false
                        SetVehicleEngineOn(vehicle, false, false, true)
                        hori.Functions.Notify("Lockpicking failed!", "error")
                        IsHotwiring = false
                        FailedAttemps = FailedAttemps + 1
                        local c = math.random(2)
                        local o = math.random(2)
...
function Hotwire()
    if not HasKey then 
        ...
        PoliceCall()
        hori.Functions.Progressbar("hotwire_vehicle", "Engaging the ignition switch", hotwireTime, false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
        ...
        if math.random(1, 100) <= chance then
            ...
            if closestPed ~= nil then
                ...
                if IsPedInAnyVehicle(GetPlayerPed(-1)) then
                    ...
                    if hori.Shared.VehicleModels[modelName] ~= nil then
                        Name = hori.Shared.Vehicles[hori.Shared.VehicleModels[modelName]["model"]]["brand"] .. ' ' .. hori.Shared.Vehicles[hori.Shared.VehicleModels[modelName]["model"]]["name"]
                    else
                        Name = "HÄTÄKESKUS"
                    end
                    local modelPlate = GetVehicleNumberPlateText(vehicle)
                    local msg = "Ajoneuvoa anastetaan " ..streetLabel.. ". Ajoneuvo: " .. Name .. ", Rekisterikilpi: " .. modelPlate
                    local alertTitle = "Ajoneuvon varkaus"
...
function GetNearbyPed()
	...
	local closestPed, closestDistance = hori.Functions.GetClosestPed(coords, PlayerPeds)
